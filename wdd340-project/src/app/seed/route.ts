...
export async function GET() {
  try {
    await sql.begin(async (tx) => {
      ...
      // ----------------------------
      // Update Mason's password properly
      // ----------------------------
      ...
      const result = await tx`
        UPDATE users
        SET password = ${hashed}
        WHERE name = 'Mason Reed'
        RETURNING id, name
      `;

      if (result.length === 0) {
        console.log("No user named 'Mason' found.");
      } else {
        console.log(`Updated password for user: ${result[0].name} (id: ${result[0].id})`);
      }

      // ----------------------------
      // Add 'name' column to artisans table if it doesn't exist
      // ----------------------------
      await tx`
        ALTER TABLE artisans
        ADD COLUMN IF NOT EXISTS name VARCHAR(35) NOT NULL DEFAULT 'Unnamed Artisan'
      `;

      console.log("Ensured 'name' column exists in artisans table.");

    });

    return Response.json({
      message: "Mason's password updated and artisans table updated successfully.",
    });
